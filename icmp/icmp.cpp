...
for (unsigned int c = 0; c < RequestCount; c++) {
	...
	typedef struct icmp_echo_reply {
		...
		Struct  IP_OPTION_INFORMATION Options;    // опции ответа 
		...
		if (pIpe->Status) {
			...
			cout << "\tПакетов: отправлено = " << RequestCount << ", поучено = " << RequestCount - LostPacketsCount << ", потеряно = " << LostPacketsCount << "<" << (int)(100 / (float)RequestCount)* LostPacketsCount << " % потерь>, " << endl;
